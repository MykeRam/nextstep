...
export default function App() {
  ...
  const [sortBy, setSortBy] = useState<'followUp' | 'appliedDate' | 'company'>('followUp');
  ...
  const visibleApplications = useMemo(() => applications.filter((application) => {
    ...
    if (sortBy === 'appliedDate') return (second.appliedDate || '').localeCompare(first.appliedDate || '');
    return (first.followUpDate || '9999-12-31').localeCompare(second.followUpDate || '9999-12-31');
  ...
  return <main className="app-shell">
    <header><div><h1>NextStep</h1><p className="subtitle">Keep your opportunities and next moves in one place.</p></div><div className="summary"><strong>{applications.length}</strong><span>applications</span></div></header>
